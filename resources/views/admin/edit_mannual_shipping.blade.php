                ...
                <div class="mw1000 center-block">
                    ...
                    <div class="allcp-form">
                        <div class="panel">
                            ...
                            <form method="post" action="{{ url('admin/shipping/manual_shipping/'.$shipping->id) }}" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                    ...
                                    <div class="col-sm-10 ph10">
                                           <!-- <input type="text" name="state" id="business-name" class="form-control" placeholder="State">-->
                                            <select name="product" class="form-control chosen-select" id="product"  >
                                                ...
                                                <option {{$shipping->vendorproduct_id == $country->product_id ? 'selected' : ''}} value="{{$country->product_id}}">{{$country->name}}({{App\User::find($country->user_id)->name}})</option>
                                                option
                                                @endforeach
                                            </select>
                                    </div>
                                    ...
                                    <div class="col-sm-10 ph10">
                                         <select name="city" class="form-control">
                                              ...
                                              <option value="" selected disabled>Select a City</option>
                                                ...
                                                @endphp
                                                @foreach($city as $cit)
                                                <option {{$shipping->city == $cit->name ? 'selected' : ''}}  value="{{$cit->name}}">{{$cit->name}}</option>
                                                @endforeach
                                         </select>
                                ...
                                <div class="section row mb10">
                                    ...
                                    <div class="col-sm-10 ph10">
                                            <input type="text" name="shipping" id="city" value="{{$shipping->shipping}}" class="form-control" placeholder="Shipping">
                                    </div>
                                </div>
