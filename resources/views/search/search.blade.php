...
<div class="container" style="width: 90%;">
    ...
    <div class="row">
        <div class="col-md-3">
                    <aside class="category-filters">
                        <div class="category-filters-section">
                            ...
                            <input type="hidden" id="category_level" value="search">
                            <input type="hidden" name="searchterm" id="searchterm" value="{{$qq}}">
                        </div>
                            <div class="category-filters-section">
                        ...
                        <form>
                            ...
                            @endif
                            <?php
                            $instancepricing=App\vendorproduct::whereIn('product_id', $search->pluck('product_id'))->where('instant_price','!=',NULL)->count();
                            $within15dayprice=App\vendorproduct::whereIn('product_id', $search->pluck('product_id'))->where('pricewithin15days','!=',NULL)->count();
                            $within30dayprice=App\vendorproduct::whereIn('product_id', $search->pluck('product_id'))->where('pricewithin30days','!=',NULL)->count();
        ...
        <div class="col-md-9">
                    ...
                    @if($key->product_status==1 && $key->delete_product==0 && $key->availability=='yes')
                    <?php
                   ...
                   $vendor=App\vendors::where('user_id','=',$key->user_id)->first();
                                    @endphp
                            <div class='product ' data-title="{{$key->id}}"
                                 data-id="{{$key->name}}"
                                 data-category="{{$category->name}}"
                                 data-model="{{$modal ? $modal->name : ''}}"
                                 data-manufactoror="{{$manufactoror ? $manufactoror->name : ''}}"
                                 data-instance="{{$instance}}" data-within15="{{$within15days}}" data-within30="{{$within30days}}"
                            data-source="{{$source}}" data-price="{{$price}}" data-condition="{{$condition}}" data-vendor="{{$vendor->vendorname}}" data-location="{{$vendor->country}}" data-battery="2700 mAH">
                                ...
                                <div class='product-caption'>
                                    ...
                                    <a class="btn btn-success addToCompare" style="position: absolute;
    right: 0;
    top: 70%;
}">Compare</a>
                                 </diV>   
                ...
                <div class="modal fade product_view" id="product_view_{{$key->product_id}}">
                                ...
                                <div class="row ">
                                         ...
                                         ?>
                                            <div class="product-page-product-wrap jqzoom-stage items">
                                                ...
                                                <div class="clearfix ">
                                                    ...
                                                    <a href="{{url('/')}}/{{$product_image->image_1}}" class="jqzoom " data-rel="gal-1">
                                    ...
                                    <div class="col-md-6 product_content">
                                        ...
                                        <?php 
                                         ...
                                         <p style="">Location: {{$vendor->address}} </p>

                                        
                                        <h5><input type="radio" name="payment" class='payment' id="radio1" value="1" checked="checked" > {{App\Http\Controllers\HomeController::converter($key->price) }} -Instant Price</h5>
                                         ...
                                         if ($customersvendor) {
                                            $outstandingpayment = DB::table('outstandingpayment')->where('user_id', Auth::user()->id)->where('payment', 'pending')->first();
                                         ...
                                         ?>
                                                ...
                                                <h5><s><input type="radio" name="payment" class='payment' value="3"  disabled> {{App\Http\Controllers\HomeController::converter($key->pricewithin30days)}}</s> - 30 days</h5>

                                            <?php
                                            }else{?>

                                                <h5><input type="radio" name="payment" class='payment' value="2" > {{App\Http\Controllers\HomeController::converter($key->pricewithin15days)}} - 15 days</h5>
                                                <h5><input type="radio" name="payment" class='payment' value="3"> {{App\Http\Controllers\HomeController::converter($key->pricewithin30days)}} - 30 days</h5>
                    ...
                    @endforeach
                     ...
                     <div class="alert aler-info">
                       	<h4 style="color: red;margin-top: 20px;">Can’t find what you are looking for <a href="{{url('create/rfq')}}" title="">click here</a>  to provide more details of the product for direct sourcing’ </h4>
                       </div>
        </div>
    </div>
</div>
