...
<script src="{{asset('js/drooltip.js')}}"></script>
	<header class="page-header">
        ...
        <ol class="breadcrumb page-breadcrumb">
            ...
            </li><li><a href="{{url('/subcategory/'.$subcategory->slog)}}">{{$subcategory->name}}</a>
            </li>
        </ol>
    	...
    	<div class="col-md-3">
                    <aside class="category-filters">
                        <div class="category-filters-section">
                            ...
                            <input type="hidden" id="category_level" value="subcategory">
                        ...
                        <form>
                            ...
                            @endif
                            <?php 
                            $instancepricing=App\vendorproduct::where('category','=',$category->id)->where('instant_price','!=',NULL)->count();
                           $within15dayprice=App\vendorproduct::where('category','=',$category->id)->where('pricewithin15days','!=',NULL)->count();
                           $within30dayprice=App\vendorproduct::where('category','=',$category->id)->where('pricewithin30days','!=',NULL)->count();
                   ...
                   $vendor=App\vendors::where('user_id','=',$key->user_id)->first();
                            ...
                            data-source="{{$source}}" data-price="{{App\Http\Controllers\HomeController::converter($key->price)}}" data-condition="{{$condition}}" data-vendor="{{$vendor->vendorname}}" data-location="{{$vendor->country}}" data-model="{{$key->model_number}}" data-serial="{{$key->serial_number}}">
                                ...
                                <div class='product-img-wrap'>
                                    ...
                                    <img class='product-img-alt' src="{{url('/')}}/{{$image}}" alt='Image Alternative text' title='Image Title' />
                                </div>
                                ...
                                <div class='product-caption'>
                                    ...
                                    <h5 class='product-caption-title'>{{$key->name}}<strong style="font-size: 10px;"><small class="{{$condition != 'New' ? 'text-danger' : '' }}">({{$condition}})</small></strong></h5>
                            ...
                            <div class="modal-body">
                                ...
                                <div class="row ">
                                    ...
                                    <div class="col-md-6 product_content">
                                        ...
                                        <h4>{{$key->name}} <small class="{{$condition != 'New' ? 'text-danger' : '' }}">({{$condition}})</small></h4>
                                        <?php
...
if($key->stock_count>$PaymentDeliveryInformation->minimum_order_quantity){
                                        ...
                                        <div class="row">
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                ...
                                                <input class="product-page-qty product-page-qty-input productqty" type="number" id="qty{{$key->id}}" value="{{$PaymentDeliveryInformation->minimum_order_quantity}}" min="{{$PaymentDeliveryInformation->minimum_order_quantity}}" size="1" maxlength="2" max="{{$max}}" onkeydown="return false">
                                        ...
                                        <div class="btn-ground item">
                                            ...
                                            if(Auth::check()){ 
                                                ...
                                                if (!empty($customersvendor)) {
                                                 ...
                                                 <div class="show">
                                                   <button type="button" class="btn btn-primary requestuserid myTooltip" title="Click this button to Send a request to this vendor, to buy and pay in 15 days or 30 days" id="<?php echo $key->id  ?>"><span class="glyphicon glyphicon-heart"></span> Request to Vendor</button>
                                                 </div>
                                              <?php  }
                                            ?>
